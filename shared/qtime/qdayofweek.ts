import { posMod } from "@schel-d/js-utils/dist/types";
...
const _cache = new QCache<QDate, QDayOfWeek>(
  (d) => new QDayOfWeek(posMod(new Date(d.year, d.month - 1, d.day).getDay() - 1, 7)),
  (d) => d.toISO(),
  100
);
...
export class QDayOfWeek {
  ...
  isValid(): { valid: true } | { valid: false; issue: string } {
    if (!Number.isInteger(this.index) || this.index < 0 || this.index > 6) {
      return { valid: false, issue: `${this.index} is not a valid day-of-week index.` };
    }
    return { valid: true };
  }
