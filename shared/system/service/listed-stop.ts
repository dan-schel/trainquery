...
export function listedStop({ stop, stopListIndex }: { stop: StopID, stopListIndex: number }): ListedStop {
  return {
    type: "served",
    stop,
    stopListIndex,
...
export function listedUnknown(stop: StopID, stopListIndex: number): ListedUnknown {
  return {
    "type": "unknown",
    stop,
    stopListIndex
...
export function getPatternList(config: HasSharedConfig, service: Service): PatternList {
  const pattern = service.pattern;
  if (pattern instanceof CompletePattern) {
    return pattern.stops.filter(s => s.stopListIndex >= pattern.origin.stopListIndex && s.stopListIndex <= pattern.terminus.stopListIndex).map(x => {
      if (!x.express) {
    ...
    const stopList = requireLine(config, service.line).route.requireStopList(service.route, service.direction);
    const trimmed = stopList.stops.filter((s, i) => i >= pattern.origin.stopListIndex && i <= pattern.terminus.stopListIndex);
    ...
    const stopList = requireLine(config, service.line).route.requireStopList(service.route, service.direction);
    const trimmed = stopList.stops.filter((s, i) => i <= pattern.terminus.stopListIndex);
    return trimmed.map((x, i) => {
