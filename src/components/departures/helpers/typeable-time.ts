...
export class TypeableTime {
  ...
  with({ hour, explicitlyDivided, minute, ampm }: { hour?: string, explicitlyDivided?: boolean, minute?: string, ampm?: string }) {
  ...
  type(_character: string): TypeableTime {
    ...
    else if ((character == ":" || character == ";") && !this.explicitlyDivided && this.hour.length == 0 && this.minute.length != 0) {
      return this.with({
        hour: this.minute,
