...
export class GtfsPersistenceConfig {
  ...
  }).transform(x => new GtfsPersistenceConfig(x.mongoUrl, x.refresh, x.refreshSeconds));
}
...
export class GtfsConfig {
  constructor(
    ...
    readonly subfeeds: string[] | null
  ) { }

  static readonly json = z.object({
    staticUrl: z.string(),
    persist: GtfsPersistenceConfig.json.nullable(),
    subfeeds: z.string().array().optional()
  }).transform(x => new GtfsConfig(x.staticUrl, x.persist, x.subfeeds ?? null));
}
